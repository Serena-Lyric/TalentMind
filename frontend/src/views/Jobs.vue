...
import { exportJobs, getJobDetail, getJobList, getJobPlatformStats, type Job, type JobPlatformStat } from '../api/jobs'
...
async function openDetail(job: Job) { detailJob.value = job; detailVisible.value = true; try { const detail = await getJobDetail(job.id); if (detail) detailJob.value = { ...job, ...detail } } catch { /* 列表数据已经可以展示 */ } }
