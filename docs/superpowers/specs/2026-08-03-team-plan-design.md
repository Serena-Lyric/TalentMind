...
- 模块间通过「文件交接 + A 唯一集成」协作：M2/M3 产出标准数据文件，M4 交付可运行模块；**接口由各模块自行定义并附接口自述，A 在集成时统一调整；先联调 MVP，再基于 MVP 二次开发**（D19）。M5 开发期使用 Mock。
...
接口由各模块自行定义（随产出物附一页接口自述：路径、入参/出参、字段含义、示例），A 在集成时统一调整。下表为 MVP 集成后 A 定稿的接口集合，供 M5 页面需求参考；开发期 M5 使用 Mock。
...
轻量惯例（D20，须在各模块计划中强调）：统一响应 `{code, message, data}`、snake_case、Mock 先行。
...
- 第 3 周：A 集成统一各模块接口，联调出 MVP（最小闭环：≥30 条 JD → 5-10 个岗位定义 → 小图谱 → 1 份简历匹配 → 三页可展示）；测试集与三项准确率验证
- 第 4 周：基于 MVP 二次开发（扩数据、演化、路径推荐）；部署、文档、演示视频
